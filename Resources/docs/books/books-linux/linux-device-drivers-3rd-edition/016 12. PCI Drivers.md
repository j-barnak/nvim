...
![Layout of a typical PCI system](/tmp/audit/iter1/epubregen/linux-device-drivers-3rd-edition/media/httpatomoreillycomsourceoreillyimages138405.png)
...
![The standardized PCI configuration registers](/tmp/audit/iter1/epubregen/linux-device-drivers-3rd-edition/media/httpatomoreillycomsourceoreillyimages138407.png)
...
`where, u32 *val);`  
Just like the *pci_read\_* functions, but `struct` `pci_bus` `*` and `devfn` variables are needed instead of a `struct` `pci_dev *`.
...
`where, u32 val);`  
Just like the *pci_write\_* functions, but `struct` `pci_bus` `*` and `devfn` variables are needed instead of a `struct pci_dev *`.

The best way to address the configuration variables using the *pci_read\_* functions is by means of the symbolic names defined in *\<linux/pci.h\>*. For example, the following small function retrieves the revision ID of a device by passing the symbolic name for `where` to *pci_read_config_byte*:
...
By making use of the *pci_resource\_* functions, a device driver can completely ignore the underlying PCI registers, since the system already used them to structure resource information.
