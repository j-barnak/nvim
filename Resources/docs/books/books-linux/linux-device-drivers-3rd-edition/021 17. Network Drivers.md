...
![How a host sees its interfaces](/tmp/audit/iter1/epubregen/linux-device-drivers-3rd-edition/media/httpatomoreillycomsourceoreillyimages138425.png)
...
Function that sets most device methods to the general-purpose implementation for Ethernet drivers. It also sets `dev->flags` and assigns the next available `eth`x name to `dev->name` if the first character in the name is a blank space or the `NULL` character.
