...
drains all Per-CPU-Pages (PCPs) to free lists.

So far we’ve covered draining of folios on a specific CPU, but not drain-

ing all CPUs. This is handled by [lru_add_drain_all()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n881) which in turn invokes

[\_\_lru_add_drain_all() .](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n790)

Examining this function, stripping out the careful synchronisation ma-
...
790 **static inline void \_\_lru_add_drain_all**(**bool** force_all_cpus) 791 {

. . .

863 **cpumask_clear**(&has_work); 864 **for_each_online_cpu**(cpu) { 865 **struct** work_struct \*work = &**per_cpu**(lru_add_drain_work, cpu); 866

867 **if** (**cpu_needs_drain**(cpu)) { 868 **INIT_WORK**(work, **lru_add_drain_per_cpu**); 869 **queue_work_on**(cpu, mm_percpu_wq, work); 870 **\_\_cpumask_set_cpu**(cpu, &has_work); 871 }

872 }

873

874 **for_each_cpu**(cpu, &has_work) 875 **flush_work**(&**per_cpu**(lru_add_drain_work, cpu));
...
This uses the [lru_add_drain_work](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n761) work queue to perform the work asyn-

chronously\*. These each in turn invoke [lru_add_drain_per_cpu()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n763) which simply

invokes, in turn, [lru_add_and_bh_lrus_drain()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n741) described above.

This function determines whether to perform a drain via
...
770 **struct** cpu_fbatches \*fbatches = &**per_cpu**(cpu_fbatches, cpu); 771

772 */\* Check these in order of likelihood that they're not zero \*/* 773 **return folio_batch_count**(&fbatches-\>lru_add) \|\| 774 **data_race**(**folio_batch_count**(&**per_cpu**(lru_rotate.fbatch, cpu)))
...
checking the equivalent for mlock folios via [need_mlock_page_drain()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/mlock.c?h=v6.0#n234) and the

buffer head equivalent in [has_bh_in_lru()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/fs/buffer.c?h=v6.0#n1408).

[\_\_lru_add_drain_all()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n790) is invoked from a number of different places, typi-

cally via lru_add_drain_all(), but is invoked directly by [lru_cache_disable()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n902)[.](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n902)

This function is used to disable folio batching altogether and atom-

ically increments the [lru_disable_count](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n892) value, which is used by the

[lru_cache_disabled()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/linux/swap.h?h=v6.0#n388) function to determine if folio batches are enabled which

in turn, if set, makes [folio_batch_add_and_move()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n257) simply immediately invoke

[folio_batch_move_lru(), ](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n232)i.e. effectively pass-through.

lru_cache_disable() is paired with [lru_cache_enable()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/linux/swap.h?h=v6.0#n393) which simply atomi-

cally increments lru_disable_count. Use of this pair allows for the systematic
...
895 *\* lru_cache_disable() needs to be called before we start compiling* 896 *\* a list of pages to be migrated using isolate_lru_page().* 897 *\* It drains pages on LRU cache and then disable on all cpus until* 898 *\* lru_cache_enable is called.* 899 *\**

900 *\* Must be paired with a call to lru_cache_enable().* 901 *\*/*

902 **void lru_cache_disable**(**void**) 903 {

904 **atomic_inc**(&lru_disable_count); 905 */\**

906 *\* Readers of lru_disable_count are protected by either disabling* 907 *\* preemption or rcu_read_lock:* 908 *\**

909 *\* preempt_disable, local_irq_disable \[bh_lru_lock()\]* 910 *\* rcu_read_lock* *\[rt_spin_lock CONFIG_PREEMPT_RT*

*\]*

911 *\* preempt_disable* *\[local_lock !CONFIG_PREEMPT_RT\]* 912 *\**

913 *\* Since v5.1 kernel, synchronize_rcu() is guaranteed to wait on* 914 *\* preempt_disable() regions of code. So any CPU which sees* 915 *\* lru_disable_count = 0 will have exited the critical* 916 *\* section when synchronize_rcu() returns.* 917 *\*/*

918 **synchronize_rcu_expedited**(); 919 **\#ifdef CONFIG_SMP**

920 **\_\_lru_add_drain_all**(**true**); 921 **\#else**

922 **lru_add_and_bh_lrus_drain**(); 923 **\#endif**
...
key operations here are incrementing [lru_disable_count](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n892) and invoking

[\_\_lru_add_drain_all() .](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/swap.c?h=v6.0#n790)

The LRU cache is disabled on memory hotplug (in [offline_pages()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/memory_hotplug.c?h=v6.0#n1791)),

on folio migration (in [do_migrate_pages()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/mempolicy.c?h=v6.0#n1085), [do_pages_move()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/migrate.c?h=v6.0#n1748) and

[\_\_alloc_contig_migrate_range()) ](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/mm/page_alloc.c?h=v6.0#n9144)and on a memory policy being applied via
