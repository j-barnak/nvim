...
- `DEFINE_PER_CPU` and `per_cpu` - macros for defining and accessing per-cpu variables
- `cpumask` - is a bitmap which represents a set of processors in the system
- `*_initcall` - macros like `early_initcall` or `fs_initcall` which register a callback for a certain kernel initialization stage
