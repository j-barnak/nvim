...
▶ Access to the I/O device registers using regular instructions ▶ Most widely used I/O method across the different
...
▶ void release_mem_region(unsigned long start, unsigned long len); ▶ Allows to prevent other drivers from requesting the same I/O registers, but is
...
can handle, because I/O memory is not mapped by default in virtual memory. ▶ The ioremap function satisfies this need:
...
▶ [devm_ioremap(),](https://elixir.bootlin.com/linux/latest/ident/devm_ioremap) [devm_iounmap()](https://elixir.bootlin.com/linux/latest/ident/devm_iounmap)

▶ [devm_ioremap_resource()](https://elixir.bootlin.com/linux/latest/ident/devm_ioremap_resource)

*•* Takes care of both the request and remapping operations!

▶ [devm_platform_ioremap_resource()](https://elixir.bootlin.com/linux/latest/ident/devm_platform_ioremap_resource)

*•* Takes care of [platform_get_resource()](https://elixir.bootlin.com/linux/latest/ident/platform_get_resource), [request_mem_region()](https://elixir.bootlin.com/linux/latest/ident/request_mem_region) and [ioremap()](https://elixir.bootlin.com/linux/latest/ident/ioremap) *•* Caution: unlike the other devm\_ functions, its first argument is of type
...
*dereferencing*) may not work on some architectures. ▶ A family of architecture-independent accessor functions are available covering
...
▶ ioread\[8/16/32/64\] and iowrite\[8/16/32/64\] are very similar to read/write

but also work with port I/O (not covered in the course), includes memory barriers ▶ ioread\[8/16/32/64\]be and iowrite\[8/16/32/64\]be for access to big-endian

devices, includes memory barriers

▶ \_\_raw_read\[b/w/l/q\] and \_\_raw_write\[b/w/l/q\] for raw access: no endianness

conversion, no memory barriers

▶ read\[b/w/l/q\]\_relaxed and write\[b/q/l/w\]\_relaxed for access to
...
*•* Write operation starts with a write memory barrier which prior writes cannot cross *•* Read operation ends with a read memory barrier which guarantees the ordering with

regard to the subsequent reads

▶ Sometimes compiler/CPU reordering is not an issue, in this case the code may be

optimized by dropping the memory barriers, using the raw or relaxed helpers
...
is the value at the corresponding physical address. ▶ Used by applications such as the X server to write directly to device memory. ▶ Easy to use from a shell with the devmem2 program ▶ For security reasons, on x86, arm, arm64, riscv, powerpc, parisc, s390:

*•* [CONFIG_STRICT_DEVMEM](https://elixir.bootlin.com/linux/latest/K/ident/CONFIG_STRICT_DEVMEM) restricts /dev/mem to non-RAM addresses (from v5.12)

*•* [CONFIG_IO_STRICT_DEVMEM](https://elixir.bootlin.com/linux/latest/K/ident/CONFIG_IO_STRICT_DEVMEM) goes beyond and only allows to access *idle* I/O ranges
