...
![An array of four \`i32\` elements, cleverly named \`array\`. Each element occupies four bytes. The start address is the address of the first byte of the first element. \`array\[0\]\` falls at byte offset 0. \`array\[3\]\` falls at byte offset 12.](media/Images/pr2e_2201.png)
...
![Two local string variables, \`pot\` and \`plate\`. \`pot\` is uninitialized, but still holds its prior pointer, capacity, and length. \`plate\` holds the string \`"pasta"\`.](media/Images/pr2e_2202.png)
...
![A vector holding one string, with capacity for one more. A variable \`soba\`, holding the string "soba". A variable \`last\`, uninitialized.](media/Images/pr2e_2203.png)
...
![A vector holding two strings, with no more capacity. A variable \`soba\`, uninitialized, but still holding the pointer, capacity, and length it held while live. A variable \`last\`, uninitialized.](media/Images/pr2e_2204.png)
...
![A gap buffer containing the text "Lord of the Rings". The buffer has capacity for 28 characters, but contains only 17. The first twelve characters appear at the left, followed by an eleven-character gap, followed by the remaining five characters.](media/Images/pr2e_2206.png)
...
![A gap buffer containing the text "Lord of the Onion Rings". The buffer has capacity for 28 characters, but contains only 23. The first 18 characters appear at the left, followed by a five-character gap, followed by the remaining five characters.](media/Images/pr2e_2207.png)
