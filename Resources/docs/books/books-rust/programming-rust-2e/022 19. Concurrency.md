...
![(Single-threaded execution of a function: a timeline)](media/Images/pr2e_1901.png)
...
![(Four threads running tasks in parallel, not in lock step, with a ragged right-hand side.)](media/Images/pr2e_1902.png)
...
![(The Mandelbrot set is shown, divided into 8 horizontal bands of equal area. The time it took to render each band is shown. The numbers range from 115 msec to 1429 msec.)](media/Images/pr2e_1904.png)
...
![(Two threads are shown. The first thread has a box labeled "sender". The second thread has a box labeled "receiver". The two boxes are connected by a line representing the channel. The first thread calls sender.send(msg) to send a message; the second thread calls receiver.recv() to receive it.)](media/Images/pr2e_1905.png)
...
![(An illustration of a Unix pipeline, showing the three separate processes that make up the pipeline. The processes overlap in time. The first process writes data to the second, and the second to the third, repeatedly throughout its execution.)](media/Images/pr2e_1906.png)
...
![(The pipeline has five stages, connected by channels.)](media/Images/pr2e_1907.png)
...
![(A Venn diagram showing a huge circle for Send and a slightly smaller one for Sync. The diagram shows that i32, bool, &str, String, TcpStream, and HashMap\<String, usize\> are both Send and Sync; Cell\<usize\> and Receiver\<u8\> are Send but not Sync; and Rc\<String\> and \*mut u8 are neither Send nor Sync.)](media/Images/pr2e_1909.png)
...
![Two threads have Rc\<String\> values that point to a shared, heap-allocated struct consisting of a reference count and the String "ouch". Thread 1 is saying "rc1.clone()", which causes an attempt to increment the reference count; thread 2 is saying "rc2.clone()" at the same time.](media/Images/pr2e_1910.png)
...
![an "Arc\<Mutex\<Receiver\<FernEvent\>\>\>" is a (1) heap-allocated, atomically reference-counted, (2) mutex-protected (3) thing that receives (4) exciting updates about ferns.](media/Images/pr2e_1911.png)
