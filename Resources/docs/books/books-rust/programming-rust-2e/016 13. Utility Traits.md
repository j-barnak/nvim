...
![Two &str references, comprising a pointer and length, point to str values. A Box\<dyn Write\>, comprising a data pointer and a vtable pointer, points to some value that implements \`std::io::Write\`. In all cases, the references are sized types, whereas their referents are unsized.](media/Images/pr2e_1301.png)
