...
One of the popular cost models for storage structures takes three factors into consideration: *R*ead, *U*pdate, and *M*emory overheads. It is called RUM Conjecture \[ATHANASSOULIS16\].
...
[Bitcask](https://databass.dev/links/82), one of the storage engines used in [Riak](https://databass.dev/links/83), is an unordered log-structured storage engine \[SHEEHY10b\]. Unlike the log-structured storage implementations discussed so far, it *does not* use memtables for buffering, and stores data records directly in logfiles.
