...
![Diagram illustrating the combination of replication and sharding, showing nodes acting as leaders for some shards and followers for others, with arrows indicating replication streams.](media/assets/ddia_0701.png)
...
![Illustration of an encyclopedia set organized by key range, with each volume labeled by the range of titles it contains, demonstrating data sharding by key range.](media/assets/ddia_0702.png)
...
A good hash function takes skewed data and makes it uniformly distributed. Say you have a 32-bit hash function that takes a string. Whenever you give it a new string, it returns a seemingly random number from 0 to 2³² − 1. Even if the input strings are very similar, their hashes are evenly distributed across that range of numbers (but the same input always produces the same output).
...
![Diagram showing key distribution changes before and after rebalancing when adding a fourth node, illustrating how keys move between nodes using a mod N hashing approach.](media/assets/ddia_0703.png)
...
![Diagram illustrating the rebalancing of shards in a database cluster before and after adding a new node, showing the reassignment of shards to distribute them evenly across the nodes.](media/assets/ddia_0704.png)
...
![Diagram illustrating the redistribution of hash ranges in Cassandra when a new node is added, showing how the dataset is balanced across nodes.](media/assets/ddia_0706.png)
...
![Diagram illustrating three methods of routing requests to nodes: 1) Direct client connection to a random node, forwarding as necessary; 2) Client connection through a routing tier to the correct node; 3) Clients aware of shard placement, connecting directly to the appropriate node.](media/assets/ddia_0707.png)
...
![Diagram illustrating how ZooKeeper manages shard assignment to nodes, showing key ranges and their corresponding nodes and IP addresses.](media/assets/ddia_0708.png)
...
![Diagram showing local secondary indexes for two database shards, highlighting how searches for red cars access specific IDs within each shard's index.](media/assets/ddia_0709.png)
...
![Diagram showing a global secondary index for car IDs, sharded by attributes such as color and make, across two primary key index shards.](media/assets/ddia_0710.png)
...
\[10\] Malte Schwarzkopf, Eddie Kohler, M. Frans Kaashoek, and Robert Morris. [“Position: GDPR Compliance by Construction.”](https://cs.brown.edu/people/malte/pub/papers/2019-poly-gdpr.pdf) At *Towards Polystores That Manage Multiple Databases, Privacy, Security and/or Policy Issues for Heterogenous Data* (Poly), August 2019. [*doi:10.1007/978-3-030-33752-0_3*](https://doi.org/10.1007/978-3-030-33752-0_3)
...
\[31\] Nadav Har’El. [“Indexing in Cassandra 3.”](https://github.com/scylladb/scylladb/wiki/Indexing-in-Cassandra-3) *github.com*, April 2017. Archived at [*perma.cc/3ENV-8T9P*](https://perma.cc/3ENV-8T9P)
