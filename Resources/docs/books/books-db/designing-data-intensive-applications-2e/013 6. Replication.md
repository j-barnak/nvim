...
![Diagram illustrating single-leader replication where user updates are written to the leader, which replicates changes to follower replicas, while reads can occur from any replica.](media/assets/ddia_0601.png)
...
![Diagram illustrating leader-based replication, showing synchronous replication to Follower 1 and asynchronous replication to Follower 2 during a profile image update process.](media/assets/ddia_0602.png)
...
![Diagram illustrating a user writing data, receiving confirmation, but reading from a stale replica, highlighting the need for read-after-write consistency.](media/assets/ddia_0603.png)
...
![Diagram illustrating how user 2345 first reads a comment from a fresh replica and then reads from a stale replica, resulting in the comment not appearing, demonstrating a time anomaly prevented by monotonic reads.](media/assets/ddia_0604.png)
...
![Diagram showing a database with 5 replicas, a write that goes to 3 out of 5, and a read that also goes to 3 out of 5 replicas. It shows that since the write and read sets overlap, at least one replica receives both the write and the read.](media/assets/ddia_0605.png)
...
![Diagram illustrating multi-leader replication across two regions, each with a leader node and follower, demonstrating how changes and conflict resolution are managed between regions.](media/assets/ddia_0606.png)
...
![Diagram showing three multi-leader replication topologies: circular, star, and all-to-all.](media/assets/ddia_0607.png)
...
![Diagram showing multi-leader replication where updates can arrive out of order, illustrating a scenario where Leader 2 receives an update prior to the corresponding insert.](media/assets/ddia_0608.png)
...
![Diagram showing a write conflict when two users concurrently update a wiki page's title to different values, illustrating conflict detection in a multi-leader database setup.](media/assets/ddia_0609.png)
...
![Diagram illustrating Amazon's shopping cart anomaly, showing how concurrent item removals by two devices lead to a conflict that results in a merged cart with reappearing items.](media/assets/ddia_0610.png)
...
![Diagram comparing how Operational Transformation and CRDT handle concurrent edits to transform the string "ice" into "nice!".](media/assets/ddia_0611.png)
...
![Diagram illustrating a quorum write where user 1234 updates a key across three replicas, with one node offline, and a subsequent read repair process when another user requests the data.](media/assets/ddia_0612.png)
...
![Diagram showing a database with 5 replicas, a write that goes to 3 out of 5, and a read that also goes to 3 out of 5 replicas. It shows that since the write and read sets overlap, at least one replica receives both the write and the read.](media/assets/ddia_0613.png)
...
![Diagram illustrating concurrent writes in a Dynamo-style datastore, showing inconsistent state across nodes due to different write orderings from two clients.](media/assets/ddia_0614.png)
...
![Diagram showing two clients concurrently adding items to a shopping cart on a shared database. Each request returns the concurrently written values that have not been overwritten by an earlier request.](media/assets/ddia_0615.png)
...
![Diagram showing causal dependencies between operations, where arrows indicate the sequence and dependency of adding items like milk, flour, eggs, and bacon.](media/assets/ddia_0616.png)
...
\[25\] Simon Willison. [Reply to: “My thoughts about Fly.io (so far) and other newish technology I’m getting into”](https://news.ycombinator.com/item?id=31413483). *news.ycombinator.com*, May 2022.
...
\[29\] Yaser Raja and Peter Celentano. [“PostgreSQL Bi-Directional Replication Using pglogical.”](https://aws.amazon.com/blogs/database/postgresql-bi-directional-replication-using-pglogical/) *aws.amazon.com*, January 2022. Archived at [*perma.cc/BUQ2-5QWN*](https://perma.cc/BUQ2-5QWN)
