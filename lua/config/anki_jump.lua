...
local function extract_snippet_after_todo(lines, row, os, oe)
  ...
  for i = row + 1, oe do
    ...
    if l:match("^%s*>") then
      ...
      if #snippet_lines > 0 then break end
    end
  end
...
function M.jump()
  ...
  if orig_h and row >= os and row <= oe and is_original_todo_line(line) then
    ...
    if not oline then
      ...
      if #anchor > 120 then anchor = anchor:sub(1, 120) end
      local found = nil
      for i = fs, fe do
  ...
  vim.notify("Not on Origin_<N>, a numbered question line, Origin: TODO, or #todo in # Original Input.", vim.log.levels.WARN)
end

vim.api.nvim_create_user_command("AnkiJump", function() M.jump() end, {})

-- Optional keymap
vim.keymap.set("n", "<leader>aj", function() M.jump() end,
  { silent = true, desc = "AnkiJump (Origin<->Question, TODO<->Original Input)" })
